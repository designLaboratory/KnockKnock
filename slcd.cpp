...
{	
	...
	PORTB->PCR[11] = PORT_PCR_MUX(7);
	PORTB->PCR[21] = PORT_PCR_MUX(7);
	PORTB->PCR[22] = PORT_PCR_MUX(7);
	PORTB->PCR[23] = PORT_PCR_MUX(7);
	PORTC->PCR[17] = PORT_PCR_MUX(7);
	PORTC->PCR[18] = PORT_PCR_MUX(7);
	PORTD->PCR[0] = PORT_PCR_MUX(7);
	PORTE->PCR[4] = PORT_PCR_MUX(7);
	PORTE->PCR[5] = PORT_PCR_MUX(7);
...
void slcd::Err(uint8_t number){
	...
	switch(number){  //4 digit depends on provided number
		case 0x00:
			...
			break;
		case 0x01:
			//LCD->WF8B[LCD_Front_Pin[6]] = (ZZZ);  //TASK 2.5 - replace ZZZ with segment definition
...
void slcd::DotHide(uint8_t digit)
{
	LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] &= ~(LCD_S_DEC);
}

void slcd::Set(uint8_t value, uint8_t digit){	

	LCD->WF8B[LCD_Front_Pin[((2*digit)-2)]] &= (LCD_C); /* To display '0' we must active segments: d, e & f on first front plane */
	LCD->WF8B[LCD_Front_Pin[((2*digit)-1)]] &= (LCD_C | LCD_S_DEC); /* To display '0' we must active segments: a, b & c on second front plane */
 ...
 *----------------------------------------------------------------------------*/
void slcd::Display(uint16_t value ,uint16_t format){	
	uint8_t val = 0;
	val = (value / (format*format*format) ) % format;
	Set(val, 1);
	val = (value / (format*format) ) % format;
	Set(val, 2);
